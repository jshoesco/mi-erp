...
const ExpenseManager = ({ config, sync }) => {
    ...
    const addCategory = () => {
        if (!newValue) return;
        const current = config.gastos || [];
        if (current.includes(newValue.toUpperCase())) return setNewValue('');
        sync({ ...config, gastos: [...current, newValue.toUpperCase()] });
        setNewValue('');
    };
    ...
    const saveEdit = (idx) => {
        ...
        const list = [...(config.gastos || [])];
        list[idx] = tempValue.toUpperCase();
        sync({ ...config, gastos: list });
        setEditingIndex(null);
    };

    const deleteItem = (idx) => {
        const list = config.gastos.filter((_, i) => i !== idx);
        sync({ ...config, gastos: list });
    };
