...
const OrderFormModal = ({
    ...
    toggleInternal,
    handlePhoneChange,
    clientHistory, // lista para datalist
    shippingOptions, // ciudades
    handleCityChange,
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input type="date" label="Fecha" value={date} onChange={e => setDate(e.target.value)} />
                ...
                <div className="space-y-3">
                    ...
                    {!isCustomMode ? (
                        <div className="relative z-20">
                            ...
                            <input 
                                className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-red outline-none" 
                                placeholder="Escribe SKU o Modelo..." 
                                value={itemSearch} 
                            ...
                            {cart.map((it, i) => (
                                <tr key={i} className="group hover:bg-gray-50">
                                    ...
                                    <td className="p-3 w-20">
                                        <input 
                                            className="w-full border border-gray-300 rounded-md px-1 py-1 text-center text-sm focus:border-brand-red outline-none" 
                                            value={it.talla || ''} 
                                            onChange={e => updateCartItem(i, 'talla', e.target.value)} 
                                            placeholder="Talla"
                                        />
                                    </td>
                                    <td className="p-3 text-right font-bold text-gray-700">{formatCurrency(it.total)}</td>
                                    <td className="p-3 text-right w-10">
                                        <button onClick={() => removeFromCart(i)} className="text-gray-300 hover:text-brand-red transition-colors p-1"><Icon name="X" size={16}/></button>
                                    </td>
                                </tr>
                            ))}
                            {cart.length === 0 && (
                                <tr><td colSpan="5" className="p-8 text-center text-gray-400 italic">El carrito está vacío.</td></tr>
                            )}
                        </tbody>
                    </table>
                    {cart.length > 0 && (
                ...
                <div className="flex justify-end gap-3 pt-2">
                    <Button variant="secondary" onClick={onClose} className="h-12 px-6">Cancelar</Button>
                    <Button onClick={onSave} className="h-12 px-8 bg-brand-dark hover:bg-black shadow-lg">
