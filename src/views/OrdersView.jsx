...
const OrdersView = () => {
    ...
    const { remitenteData } = useData(); // <-- Importar remitenteData
    ...
    const deleteGuide = async () => {
        confirmAction({
            title: "Borrar Guía", message: "Se eliminará la deuda de envío. ¿Seguro?", onConfirm: async () => {
                try {
                    const batch = writeBatch(db);
                    guideForm.targetItems.forEach(t => {
                        const fullOrder = orders.find(o => o.id === t.orderId);
    ...
    // FUNCIÓN 3: EJECUTAR UNIÓN (FINAL)
    const submitLinkOrders = async (forcedAddress = null) => {
        // Usamos la dirección forzada (caso automático) o la seleccionada en el modal
        const addressToSave = forcedAddress || selectedAddress;
        ...
        try {
            ...
            linkSourceGroup.items.forEach(item => {
                const ref = doc(db, 'pedidos', item.orderId);
                const fullOrder = orders.find(o => o.id === item.orderId);
                
                if (fullOrder) {
                    // 1. Vinculamos los items
                    const newItems = fullOrder.items.map((it, idx) => {
                        if (idx === item.itemIndex) return { ...it, linked_to_order_id: selectedTargetId };
                        return it;
                    });
                    
                    // 2. Preparamos actualización
                    const updateData = { items: newItems };

                    // 3. ¡AQUÍ ESTÁ LA MAGIA! Guardamos la dirección elegida como "datos_envio"
                    // Así el rótulo la detectará automáticamente por defecto.
                    if (addressToSave) {
                        updateData.datos_envio = {
                            nombre: addressToSave.clientName,
                            direccion: addressToSave.value, // La dirección elegida
                            telefono: addressToSave.clientPhone,
                            ciudad: addressToSave.clientCity,
                            cedula: '' // Se llenará si el usuario la edita en el rótulo después
                        };
                    }
            ...
            await batch.commit();
            notify("Pedidos unificados correctamente");
            
            // CERRAR TODOS LOS MODALES
            setLinkModalOpen(false);
            setAddressConflictOpen(false); // <--- ESTO ARREGLA QUE NO SE CERRABA
                    ...
                    {columnType === 'ready' && (
                        <div className="flex gap-2">
                            ...
                            <button 
                                onClick={(e) => {
                                    e.stopPropagation();
                                    const enrichedItems = group.items.map(it => {
                                        const parentOrder = orders.find(o => o.id === it.orderId);
                                        return {
                                            ...it,
                                            // 1. Datos del Cliente (Dueño)
                                            clientName: parentOrder?.cliente?.nombre,
                                            clientPhone: parentOrder?.cliente?.telefono,
                                            clientAddress: parentOrder?.cliente?.direccion,
                                            clientCity: parentOrder?.cliente?.ciudad_entrega,
                                            
                                            // 2. Datos de Envío (Destinatario específico, si ya se guardó antes)
                                            shippingData: parentOrder?.datos_envio || null 
                                        };
                                    });
                                    setShippingItems(enrichedItems);
                                    setShippingModalOpen(true);
                                }} 
                                className="flex-1 py-2 bg-white border border-indigo-200 text-indigo-700 text-xs font-bold rounded-lg hover:bg-indigo-50 flex items-center justify-center gap-2 transition-all"
                            >
                                <Icon name="FileText" size={14}/> Rótulo
                            </button>

                            {/* BOTÓN EXISTENTE: ASIGNAR GUÍA */}
                            <button
                                onClick={(e) => { e.stopPropagation(); openGuide(group.items) }}
                                className="flex-1 py-2 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 flex items-center justify-center gap-2 shadow-sm transition-all"
                            >
                                <Icon name="Truck" size={14} /> Guía
                            </button>
                        </div>
                    )}
                </div>
    ...
    return (
        <div className="h-full flex flex-col fade-in pb-20 md:pb-0">
            ...
            {/* Modal Conflicto Dirección (Busca este bloque al final del archivo) */}
            <Modal isOpen={addressConflictOpen} onClose={() => setAddressConflictOpen(false)} title="Dirección de Entrega">
                <div className="space-y-4">
                    <p className="text-sm text-gray-600">
                        Los pedidos tienen direcciones diferentes. ¿Cuál quieres usar para el envío unificado?
                    </p>
                    ...
                    <div className="space-y-2">
                        {addressOptions.map((opt, i) => (
                            <div 
                                key={i} 
                                onClick={() => setSelectedAddress(opt)}
                                className={`p-3 border rounded-lg cursor-pointer flex items-center gap-3 transition-colors ${selectedAddress?.value === opt.value ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-500' : 'hover:bg-gray-50 border-gray-200'}`}
                            >
                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedAddress?.value === opt.value ? 'border-indigo-600' : 'border-gray-400'}`}>
                                    {selectedAddress?.value === opt.value && <div className="w-2 h-2 bg-indigo-600 rounded-full"></div>}
                                </div>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        {/* ESTA ES LA LÍNEA DEL PUNTO 3 QUE DEBES REVISAR: */}
